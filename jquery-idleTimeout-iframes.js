...
(function($) {
  ...
  $.fn.idleTimeout = function(options) {
    ...
    var checkForIframes = function() {
      ...
      var isDocReady = setInterval(docReadyCheck, 1000);
    };

    // look for iframes
    var includeIframes = function() {

      var foundIframes = document.getElementsByTagName('iframe');

      if (foundIframes.length > 0) { //at least one iframe found

        // attach events to each iframe found
        for (var index = 0; index < foundIframes.length; index++) {

          var iframeItem = foundIframes.item(index);
    ...
    var attachEventIframe = function(index) {
